...
function fieldHtml(f,val){
  ...
  if(f.type==='toggle'){
    const on=val!=null?!!val:!!f.value;
    return `<div class="field field-toggle"><label class="inline-check"><input type="checkbox" name="${f.name}" ${on?'checked':''}> ${esc(f.label)}</label>${f.hint?`<small class="muted">${esc(f.hint)}</small>`:''}<small class="err"></small></div>`;
  }
  let ctl;
...
function openForm({title,sub='',fields,values={},submit='Lưu',wide=false,onSubmit}){
  ...
  form.addEventListener('submit',async e=>{
    ...
    fields.forEach(f=>{
      ...
      else if(f.type==='toggle')v=el.checked;
      ...
      if(f.req&&(f.type==='checks'?!v.length:f.type==='toggle'?!v:!v))msg='Vui lòng điền mục này';
      else if(f.validate&&(v||f.type==='checks'||f.type==='toggle'))msg=f.validate(v,d)||'';
...
const COURSE_LEVEL_BASE={'Cơ bản':129000,'Trung cấp':199000,'Nâng cao':299000};
const COURSE_LEVEL_PTS={'Cơ bản':15,'Trung cấp':25,'Nâng cao':40};
COURSES.forEach(c=>{
  c.weeks=parseInt(c.duration,10)||4;
  const bump=c.weeks<=4?0:c.weeks<=6?30000:c.weeks<=8?60000:90000;
  c.price=COURSE_LEVEL_BASE[c.level]+bump;
  c.completePts=COURSE_LEVEL_PTS[c.level]+c.weeks*3;
});
const uehCoursePrice=p=>Math.round(p*0.6/1000)*1000;
const coursePriceFor=(c,u)=>isUEH(u)?uehCoursePrice(c.price):c.price;
...
async function seedIfNeeded(){
  ...
  S.users=[
    ...
    await mk({id:'u_ueh',role:'student',name:'Trần Thiện An',email:'antran.31261027349@st.ueh.edu.vn',courses:[],saved:[],connections:[],interviews:[],roadmapDone:{},
      profile:{school:'Đại học Kinh tế TP.HCM (UEH)',major:'',gradYear:'2026',targetRole:'Business Analyst',bio:'',gpa:3.46,transcripts:[],
        certs:[{id:uid(),name:'Google Data Analytics'},{id:uid(),name:'MOS Excel Associate'},{id:uid(),name:'IELTS 6.5'}],
        hardSkills:['Excel','Phân tích dữ liệu','BPMN'],softSkills:['Thuyết trình','Làm việc nhóm','Tư duy phản biện','Quản lý thời gian'],
        projects:[{id:uid(),name:'Nghiên cứu hành vi mua sắm xanh của sinh viên'},{id:uid(),name:'Dashboard phân tích doanh thu thương mại điện tử'},{id:uid(),name:'Thiết kế quy trình tuyển dụng bằng BPMN'}],
...
function initAuth(){
  ...
  $('auth').addEventListener('click',e=>{
    ...
    if(demo){const f=$('loginForm'),map={student:'tranthienanpct@gmail.com',ueh:'antran.31261027349@st.ueh.edu.vn',employer:'hr@digiflow.vn'};f.elements.email.value=map[demo.dataset.demo];f.elements.password.value='123456';f.requestSubmit()}
  });
...
function renderProfile(){
    ...
    <div class="two" style="margin-top:0">
      ...
      <div>
        ...
        ${!coursesOK?`<div class="lock-panel" style="padding:20px 16px"><div class="lock-ico" aria-hidden="true">🔒</div><p class="muted" style="margin:0 0 12px">Mở khóa để Lodestar AI đề xuất và đăng ký khóa học theo kỹ năng còn thiếu.</p><button type="button" class="btn orange sm" data-go="pricing">Xem gói nâng cấp</button></div>`
          :a.recs.length?`<div class="list">${a.recs.map(r=>`<div class="rec"><div><b>${esc(r.course.title)}</b><small>${r.pct}% phù hợp · ${esc(r.course.duration)} · bổ sung ${esc(r.fill.map(labelOf).join(', '))}</small></div>${courseActionHtml(r.course,u,{sm:true})}</div>`).join('')}</div>`:`<p class="muted">Không có khóa học cần bổ sung ở mục tiêu này.</p>`}
      </div>
    </div>`}
  </div>
  ...
  ${aiBusy?'':roadmapOK?`<div class="card" style="margin-top:16px"><div class="row-head"><h3>Lộ trình 12 tuần dành riêng cho bạn</h3><span class="chip">${dn}/${a.roadmap.length} giai đoạn hoàn thành</span></div>
    ...
    <ol class="road" style="margin-top:14px">${a.roadmap.map(s=>`<li class="${done[s.key]?'done':''}"><input type="checkbox" id="rd_${s.key}" data-change="road" data-key="${s.key}" ${done[s.key]?'checked':''}><label for="rd_${s.key}"><b>Tuần ${s.from===s.to?s.from:s.from+'-'+s.to}: ${esc(s.title)}</b><small>${esc(s.detail)}</small></label>${s.courseId?courseActionHtml(COURSES.find(x=>x.id===s.courseId),u,{sm:true}):''}</li>`).join('')}</ol></div>`
...
/* course-reg: xem js7_plans.js (openCourseCheckout, có giảm giá SV UEH và dùng điểm thưởng) */
...
const CONTACT_EMAIL='tranthienanpct@gmail.com';
...
function ensureUserDefaults(u){
  if(u.role==='student'){
    ...
    u.grants=u.grants||{};u.usage=u.usage||{};u.billing=u.billing||[];u.savedPosts=u.savedPosts||[];u.cvs=u.cvs||[];u.trialUsed=!!u.trialUsed;u.coursesDone=u.coursesDone||[];
...
function openUehInfo(){
  ...
  <p class="muted">Đăng ký bằng email trường dạng <b>hoten.mssv@st.ueh.edu.vn</b> để mở toàn bộ chức năng, không mất phí: Lodestar không giới hạn, phòng mô phỏng phỏng vấn, đề xuất khóa học, lộ trình 12 tuần và các tin tuyển dụng dành riêng cho sinh viên UEH.</p>
...
function openMyPlan(){
  ...
  if(u.role==='employer'){
    ...
    <p class="notice" style="margin:12px 0 0">Hỗ trợ doanh nghiệp: ${hotlineLink()} · ${emailLink()}</p>
...
/* ---------- Thanh toán khóa học (giảm giá SV UEH 40%, dùng điểm thưởng) ---------- */
function openCourseCheckout(id){
  const u=me();
  if(!needStudent('đăng ký khóa học',()=>openCourseCheckout(id)))return;
  if(!can(u,'courses')&&!isUEH(u)){openUpsell('courses');return}
  const c=COURSES.find(x=>x.id===id);if(!c)return;
  if((u.courses||[]).includes(id)){toast('Bạn đã đăng ký khóa học này rồi.','warn');return}
  const ueh=isUEH(u),base=c.price,afterUeh=coursePriceFor(c,u),maxPts=Math.min(u.points||0,Math.floor(afterUeh/1000));
  let usePts=false;
  const finalPrice=()=>Math.max(0,afterUeh-(usePts?maxPts*1000:0));
  const m=modal(`<h2>Đăng ký khóa học</h2><p class="muted">${esc(c.title)} · ${esc(c.level)} · ${esc(c.duration)}</p>
   <div class="sumbox">
    <div><span>Giá khóa học</span><b>${vnd(base)}</b></div>
    ${ueh?`<div><span>Ưu đãi sinh viên UEH (-40%)</span><b style="color:var(--ok)">-${vnd(base-afterUeh)}</b></div>`:''}
    <div id="ckPtsRow" class="hide"><span>Dùng điểm thưởng</span><b id="ckPtsVal"></b></div>
    <div><span>Thành tiền</span><b id="ckFinal" style="font-size:17px;color:var(--t)"></b></div>
   </div>
   ${maxPts>0?`<label class="inline-check field-toggle"><input type="checkbox" id="ckUsePts"> Dùng ${maxPts} điểm thưởng để giảm ${vnd(maxPts*1000)}${maxPts*1000>=afterUeh?' (miễn phí toàn bộ khóa học)':''}</label>`:`<p class="notice">Bạn có ${u.points||0} điểm thưởng. Tích thêm điểm ở mục Điểm thưởng để giảm giá khóa học.</p>`}
   ${payMethodsHtml()}
   <div class="modal-actions"><button type="button" class="btn outline" data-close>Hủy</button><button type="button" class="btn orange" data-pay>Xác nhận thanh toán</button></div>`);
  const payBtn=qs('[data-pay]',m);
  const upd=()=>{
    const fp=finalPrice();
    $('ckFinal').textContent=fp===0?'Miễn phí':vnd(fp);
    const row=$('ckPtsRow');if(usePts){row.classList.remove('hide');$('ckPtsVal').textContent='-'+vnd(maxPts*1000)}else row.classList.add('hide');
    payBtn.textContent=fp===0?'Đăng ký miễn phí':'Xác nhận thanh toán';
  };
  upd();
  const cb=qs('#ckUsePts',m);if(cb)cb.addEventListener('change',e=>{usePts=e.target.checked;upd()});
  payBtn.addEventListener('click',e=>{
    const b=e.currentTarget;b.disabled=true;b.textContent='Đang xử lý...';
    const spend=usePts?maxPts:0,fp=finalPrice();
    setTimeout(()=>{
      const set=new Set(u.courses||[]);set.add(c.id);u.courses=[...set];
      if(spend){u.points=(u.points||0)-spend;u.pointLog.unshift({at:Date.now(),text:`Dùng điểm giảm giá khóa "${c.title}"`,pts:-spend})}
      u.billing.unshift({at:Date.now(),text:`Khóa học: ${c.title}`,amount:fp,note:fp===0?'Đăng ký miễn phí bằng điểm thưởng':'Đã thanh toán (mô phỏng)'});
      persist('users');notify(u.id,`Bạn đã đăng ký khóa học "${c.title}".`);
      closeEl(m);toast(fp===0?`Đã đăng ký miễn phí "${c.title}" bằng điểm thưởng.`:`Đã đăng ký "${c.title}".`);
      renderNav();refresh();
    },900);
  });
}
ACT['course-reg']=t=>openCourseCheckout(+t.dataset.id);
ACT['course-unreg']=async t=>{
  const u=me(),id=+t.dataset.id,c=COURSES.find(x=>x.id===id);if(!u||!c)return;
  if(!await confirmBox(`Hủy đăng ký khóa "${c.title}"? Bạn có thể phải đăng ký và thanh toán lại nếu muốn học lại.`,'Hủy đăng ký',true))return;
  u.courses=(u.courses||[]).filter(x=>x!==id);persist('users');toast('Đã hủy đăng ký khóa học.');refresh();
};
function completeCourse(id){
  const u=me();if(!u||u.role!=='student')return;
  const c=COURSES.find(x=>x.id===id);if(!c)return;
  if(!(u.courses||[]).includes(id)){toast('Hãy đăng ký khóa học này trước khi đánh dấu hoàn thành.','warn');return}
  u.coursesDone=u.coursesDone||[];if(u.coursesDone.includes(id))return;
  u.coursesDone.push(id);u.points=(u.points||0)+c.completePts;
  u.pointLog.unshift({at:Date.now(),text:`Hoàn thành khóa "${c.title}"`,pts:c.completePts});
  persist('users');renderNav();toast(`Chúc mừng! +${c.completePts} điểm thưởng vì hoàn thành "${c.title}".`);
  refresh();
}
ACT['course-done']=t=>completeCourse(+t.dataset.id);
function courseActionHtml(c,u,{sm=false}={}){
  const cls=sm?' sm':'';
  if(!u)return `<button type="button" class="btn orange${cls}" data-act="login">Đăng nhập</button>`;
  if(!(can(u,'courses')||isUEH(u)))return `<button type="button" class="btn outline${cls}" data-act="upsell" data-f="courses">🔒 Nâng cấp</button>`;
  if((u.coursesDone||[]).includes(c.id))return `<span class="chip ok">🎉 Đã hoàn thành +${c.completePts}đ</span>`;
  if((u.courses||[]).includes(c.id))return `<button type="button" class="btn outline${cls}" data-act="course-done" data-id="${c.id}">Đánh dấu hoàn thành</button>`;
  return `<button type="button" class="btn orange${cls}" data-act="course-reg" data-id="${c.id}">Đăng ký · ${vnd(coursePriceFor(c,u))}</button>`;
}
...
function renderPricing(){
  ...
  <div class="card contact-card"><div><h3 style="margin:0 0 4px">Cần tư vấn gói doanh nghiệp?</h3><p class="muted" style="margin:0">Gọi hotline hoặc gửi email để được hỗ trợ đăng ký, xuất hóa đơn VAT hoặc chọn gói phù hợp.</p><p class="muted" style="margin:4px 0 0">✉ ${emailLink()}</p></div><a class="btn primary" href="tel:${HOTLINE.raw}">📞 Hotline ${HOTLINE.text}</a></div>
...
function jobCard(j,ctx){
  ...
  const isEmp=u&&u.role==='employer';
  const heart=isEmp?'':`<button type="button" class="job-heart" data-act="job-save" data-id="${j.id}" aria-pressed="${saved}" aria-label="${saved?'Bỏ lưu tin này':'Lưu tin này'}">${saved?'♥':'♡'}</button>`;
  const acts=isEmp
    ?`<button type="button" class="btn outline" data-act="job-open" data-id="${j.id}">Xem chi tiết</button>`
    :`<button type="button" class="btn outline" data-act="hr-chat" data-id="${j.id}">Chat HR</button><button type="button" class="btn ${applied?'registered':'orange'}" data-act="job-apply" data-id="${j.id}" ${applied||closed?'disabled':''}>${applied?'Đã ứng tuyển':closed?'Đã đóng':'Ứng tuyển'}</button>`;
  return `<article class="card job">${heart}<div class="logoBox" aria-hidden="true">${esc(j.company[0])}</div><div class="jobmain"><h3><button type="button" data-act="job-open" data-id="${j.id}">${esc(j.title)}</button></h3><span class="co">${esc(j.company)}</span>
  ...
  <div class="chips"><span class="chip">${esc(j.type)}</span><span class="chip">${esc(j.workplace)}</span>${sk}${matchChip(j,ctx)}${j.uehOnly?'<span class="chip ok">🎓 Dành cho SV UEH</span>':''}${own?'<span class="chip ok">Tin của bạn</span>':''}</div>
  <div class="job-actions-row">${acts}</div></div></article>`;
}
...
function toggleSave(id){
  ...
  qsa(`[data-act="job-save"][data-id="${id}"]`).forEach(b=>{
    b.textContent=b.classList.contains('job-heart')?(on?'♥':'♡'):(on?'♥ Đã lưu':'♡ Lưu tin');
    b.setAttribute('aria-pressed',on);b.setAttribute('aria-label',on?'Bỏ lưu tin này':'Lưu tin này');
  });
...
function openJobForm(id){
  ...
  openForm({title:ex?'Sửa tin tuyển dụng':'Đăng tin tuyển dụng',sub:`Tin sẽ hiển thị với tên ${esc(u.name)} và nằm đầu danh sách Việc làm.`,wide:true,values:vals,submit:ex?'Lưu thay đổi':'Đăng tin',fields:[
    ...
    {name:'uehOnly',label:'Chỉ dành cho sinh viên UEH (email @st.ueh.edu.vn)',type:'toggle',hint:'Sinh viên trường khác sẽ thấy tin này ở dạng bị làm mờ.'},
    ...
    onSubmit:d=>{
      const o={title:d.title,type:d.type,workplace:d.workplace,city:d.city,salary:d.salary,skills:d.skills,desc:d.desc,reqs:lines(d.reqs),benefits:lines(d.benefits),deadline:d.deadline?new Date(d.deadline+'T00:00:00').getTime():null,uehOnly:!!d.uehOnly};
...
function renderCourses(){
  ...
  $('courseList').innerHTML=list.slice((coursePage-1)*8,coursePage*8).map(c=>{
    const on=regs.has(c.id),done=st&&(u.coursesDone||[]).includes(c.id),fills=coursesOK&&a&&a.gaps.length&&c.sk.some(i=>a.gaps.includes(i));
    const ueh=st&&isUEH(u),price=coursePriceFor(c,u);
    const priceLine=!st?`<b>${vnd(c.price)}</b>`:done?`<span class="chip ok">🎉 Đã hoàn thành</span>`:on?`<span class="chip ok">✓ Đã đăng ký</span>`:ueh?`<span><s class="muted" style="font-weight:500">${vnd(c.price)}</s> <b style="color:var(--ok)">${vnd(price)}</b> <small class="chip ok" style="padding:2px 7px">-40% SV UEH</small></span>`:`<b>${vnd(price)}</b>`;
    return `<article class="card course-card"><div class="course-cover"><div><small>${esc(c.level)} · ${esc(c.duration)}</small><div><b>${coursesOK?score(c)+'% phù hợp':'Khóa học nổi bật'}</b></div></div><span class="course-icon" aria-hidden="true">${c.icon}</span></div><h3>${esc(c.title)}</h3><p class="course-desc">${esc(c.desc)}</p><div class="chips">${c.skills.map(x=>`<span class="chip">${esc(x)}</span>`).join('')}${fills?'<span class="chip ok">✦ Bổ sung kỹ năng còn thiếu</span>':''}</div><div class="course-route"><b>Lộ trình tổng quát</b><br>${esc(c.route)}</div><div class="course-price">${priceLine}</div><div class="course-footer"><small>${esc(c.duration)}</small>${courseActionHtml(c,st?u:null)}</div></article>`;
...
async function startCamera(){
  try{
    ...
    I.mic=true;$('muteBtn').textContent='🎙️';$('muteBtn').setAttribute('aria-label','Tắt mic');$('muteBtn').setAttribute('aria-pressed','false');
...
function initInterview(){
  ...
  $('muteBtn').addEventListener('click',()=>{
    I.mic=!I.mic;I.stream&&I.stream.getAudioTracks().forEach(t=>t.enabled=I.mic);
    const b=$('muteBtn');b.textContent=I.mic?'🎙️':'🔇';b.setAttribute('aria-label',I.mic?'Tắt mic':'Bật mic');b.setAttribute('aria-pressed',String(!I.mic));
  });
